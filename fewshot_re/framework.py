        ...
        for it in range(start_iter, start_iter + train_iter):
            if pair:
                ...
                logits, pred  = model(support, query, 
                        N_for_train, K, Q * N_for_train + na_rate * Q, templates)
                ...
                if pair:
                    ...
                    if torch.cuda.is_available():
                        for k in batch:
                            batch[k] = batch[k].cuda()
                        label = label.cuda()
                    logits, pred = model(batch, N, K, Q * N + Q * na_rate, templates)
                else:
                    ...
                    if torch.cuda.is_available():
                        ...
                        for k in query:
                            ...
                            query[k] = query[k].cuda()
                        label = label.cuda()
                    logits, pred = model(support, query, N, K, Q * N + Q * na_rate, templates)
