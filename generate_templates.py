...
def get_text(template, input_text_tuple, label, tokenizer, mapping):
    ...
    for part in template_list:
        ...
        if part in special_token_mapping:
            ...
            new_tokens += enc(' ' + mapping[label])  # mapping list, save all the classifications
        elif part[:5] == 'sent_':
...
def load_dataset(task, data_dir):
    if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "NYT", "SST-2", "STS-B", "WNLI", "CoLA"]:
...
def search_template(model, tokenizer, task_name, k, seed, beam, output_dir, data_dir):
    ...
    map_of_mapping = {
        ...
        'NYT': { '0': "Kill", '1': "OrgBased_In", '2': "Live_In", '3': "Located_In", '4': "Work_For"},
    ...
    if task_name in ['NYT', 'SST-2', 'sst-5', 'mr', 'cr', 'subj', 'trec', 'CoLA', 'mpqa']:
        # Single sentence tasks
        # We take two kinds of templates: put [MASK] at the beginning or the end
        # template1 = "*cls**sentu_0**<extra_id_0>**label**<extra_id_1>**sep+*"  # template1
        template = "*cls**sentu_0**<extra_id_0>*relationship between two entities is*<extra_id_1>**label**<extra_id_2>**sep+*"  # template2
        generate_text = generate(dataset, template, model, tokenizer, target_number=2, mapping=mapping, beam=beam,
                                 label=None, truncate='head')[:beam]

        print("####### generated templates #######")
        for text in generate_text:
            # Transform T5 outputs to our template format,
            # conversion 1
            # eg: generated text <extra_id_0>▁I▁thought▁it▁was<extra_id_1>!</s>
            # after replace :      *cls**sent_0*_I_thought_it_was*mask*!*sep+*
            print("The generate text: %s" % text)
            text = text.replace('<extra_id_0>', '[cls] sent_0 s')
            text = text.replace('<extra_id_1>', '[mask] is')
            text = text.replace('<extra_id_2>', '_[E1]_and_[E2].')
            text = text.replace('▁', '_')
            print(text)
            f.write(text + '\n')
        print("####### generated templates #######\n")
...
def main():
    ...
    parser.add_argument('--task_name', type=str, nargs='+',
                        default=['NYT'], help="Task names")
    # parser.add_argument('--task_name', type=str, nargs='+',
    #                     default=['NYT', 'SST-2', 'sst-5', 'mr', 'cr', 'subj', 'trec', 'CoLA', 'MRPC', 'QQP',
    #                              'STS-B', 'MNLI', 'SNLI', 'QNLI', 'RTE'], help="Task names")
