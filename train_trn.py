from fewshot_re.data_loader import get_loader, get_loader_pair, get_loader_unsupervised
from fewshot_re.framework import FewShotREFramework
from fewshot_re.sentence_encoder import CNNSentenceEncoder, BERTSentenceEncoder, BERTPAIRSentenceEncoder
from TRNmodel.trn import TRN
from TRNmodel.d import Discriminator
import sys
import torch
...
def main():
    ...
    parser.add_argument('--test', default='test_wiki',
            help='test file')
    parser.add_argument('--templates', default='auto_templates',
                        help='templates file')
    ...
    parser.add_argument('--model', default='TRN',
            help='model name')
    ...
    with open('./data/auto_templates.txt', 'r') as ft:
        templates_data =[]
        for line in ft:
            template = line.strip().split('\t')[0].replace("[cls] sent_0 s_", "")
    ...
    # End
    model = TRN(sentence_encoder, hidden_size=opt.hidden_size, eps=opt.eps, temp=opt.temp, step=opt.step, smp=opt.smp, ratio=opt.ratio, wtp=opt.wtp, wtn=opt.wtn, wtb=opt.wtb, metric=opt.metric)
    ...
    if not opt.only_test:
        ...
        framework.train(model, prefix, batch_size, trainN, N, K, Q, templates_data,
    ...
    acc = framework.eval(model, batch_size, N, K, Q, templates_data, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair)
